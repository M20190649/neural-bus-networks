...
def parse_mbta_xml(database, thefile, presentData = None):
    ...
    try:
        ...
        root = tree.getroot()
    except (IOError, ET.ParseError): #Error parsing XML content of the file
    ...
    #Part 1.5: Purge all hashes that are too old (assume that data is read in chronological order)
    #for vehicle, time in presentData.keys():
    #	if report_time - time > 300: #5 minutes
...
def ParseAll(theHDF5FileName = 'mbta_trajectories.h5', Cleanup = True):
    ...
    try:
	    ...
	    expectedrows=2*500*len(glob.glob('*.xml'))) #Create table indexers
...
if __name__ == '__main__':
    import datetime
    #Rotate database on weekly basis.
    #Record up to 4 am into the first day of week in previous week
    now = datetime.datetime.now() - datetime.timedelta(hours=4)
    now2=now.isocalendar()
    thefilename='mbta_trajectories_%d_%02d.h5' % now.isocalendar()[0:2]
    ParseAll(thefilename)
