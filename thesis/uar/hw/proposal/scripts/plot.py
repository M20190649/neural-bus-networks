...
for this_stop in range(int(stop_idxs[-1])):
    ...
    ax.hist(gaps, xdata, normed = 1, color='#c0392b')
    #ax.bar(xdata, ydata)
    ax.set_xlabel('Time between buses (min) at Mass Ave and Newbury')
    ...
    p.plot(t, [popt[0]*Q(x, popt[2]) for x in t], linewidth=4, color='#2980b9')
